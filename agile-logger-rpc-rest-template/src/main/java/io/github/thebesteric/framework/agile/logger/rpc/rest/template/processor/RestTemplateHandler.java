...
public class RestTemplateHandler implements ClientHttpRequestInterceptor {
    ...
    public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution) throws IOException {
        ...
        interceptRequest(request, body, requestLog);
        ...
        try {
            ...
            InvocationHandler invocationHandler = new ClientHttpResponseHandler(response, requestLog);
    ...
    private void interceptRequest(HttpRequest request, byte[] body, RequestLog requestLog) {
        requestLog.setMethod(request.getMethodValue().toUpperCase(Locale.ROOT));

        // Uri & Url
        URI uri = request.getURI();
        requestLog.setUrl(UrlUtils.urlDecode(uri.toString()));
        String[] arr = uri.toString().split("//");
        if (arr.length > 1) {
            requestLog.setUri(UrlUtils.urlDecode(arr[1].substring(arr[1].indexOf("/"))));
        }
        ...
        requestLog.setDomain(UrlUtils.urlDecode(uri.toString()));

        // Request body
        requestLog.setRawBody(CollectionUtils.isNotEmpty(body) ? UrlUtils.urlDecode(StringUtils.toStr(body)) : null);
